...
const botToken = "7377106182:AAGOS0_xvyFiG8eU8zTtb6Cde3hjB41Z7Wc";
const port = 8000;
...
bot.on(":text", async (ctx) => {
  ...
  if (message.text === "Join our community") {
    return await ctx.reply(
      "Click here to join our main channel:    https://t.me/+5IyRGnhrSvcxNmU0",
      { reply_markup: keyboard }
    );
  }
  if (message.text === "Invite Friends") {
    return await ctx.reply(
      "Click here to invite your friends:    https://t.me/+5IyRGnhrSvcxNmU0",
      { reply_markup: keyboard }
    );
  }
  if (message.text === "Register and get 300 Birr reward") {
    return await ctx.reply(
      "Enter your bank account to get 300 Birr reward",
      { reply_markup: keyboard }
    );
  }
...
// docker build -t smartfx .
// docker run -p 8000:8000 smartfx